...
class OAuthLoginView(APIView):
    ...
    def post(self, request, provider, *args, **kwargs):
        ...
        if not access_token:
            return Response({'error': 'Access token not provided or improperly formatted.'},
        ...
        if not user_info:
            return Response({'error': f'Invalid or expired {provider} access token.'},
                            status=status.HTTP_400_BAD_REQUEST)

        user = handler.get_or_create_user(user_info)
        if not user:
            return Response({'error': 'User could not be created or retrieved.'},
